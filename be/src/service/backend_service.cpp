...
Status _download_file_from_peer(const std::string& peer_host, const std::string& peer_http_port,
                                ...
                                std::vector<std::string>& download_success_files) {
    auto remote_file_url = fmt::format("http://{}:{}/api/_tablet/_download?token={}&file={}&channel=ingest_binlog",
    ...
    auto download_cb = [&remote_file_url, &local_path, &peer_host, &remote_path, file_size,
                        estimate_timeout, &expected_md5, &download_success_files](HttpClient* client) {
...
void _ingest_binlog_from_peer_impl(StorageEngine& engine, const TIngestBinlogRequest& request,
    ...
    if (request.files.empty()) {
        ...
        auto commit_result = commit_ingested_rowset(engine, local_tablet, txn_id, partition_id,
                                                    rowset_meta, std::move(pending_rs_guard), watch,
                                                    elapsed_time_map);
        ...
        if (commit_result == IngestCommitResult::kAlreadyExist) {
            ...
            LOG(INFO) << "ingest binlog from peer empty rowset already committed, txn_id=" << txn_id;
        }
    ...
    for (const auto& file : request.files) {
        ...
        if (file.__isset.is_index_file && file.is_index_file) {
            ...
            local_path = local_segment_path(local_tablet->tablet_path(),
                                            rowset_meta->rowset_id().to_string(),
                                            file.segment_index);
        }
...
Status _distribute_ingested_rowset_to_followers(
    ...
    for (const auto& follower : request.follower_replicas) {
        if (!follower.__isset.backend_id || !follower.__isset.host ||
            !follower.__isset.be_port) {
        ...
        auto worker = [promise_ptr, backend_id, host, be_port, timeout_ms, &request,
            ...
            try {
                ...
                Status status = ThriftRpcHelper::rpc<BackendServiceClient>(
                        host, be_port,
                        [&follower_request, &follower_result](
                ...
                promise_ptr->set_value(
                        std::make_pair(backend_id, Status::InternalError(e.what())));
            }
        };
...
void _ingest_binlog(StorageEngine& engine, IngestBinlogArg* arg) {
    ...
    Defer defer {[=, &engine, &tstatus, ingest_binlog_tstatus = arg->tstatus, &watch,
                  ...
                  &download_success_files, &committed, &commit_already_exist,
        ...
        if (tstatus.status_code != TStatusCode::OK && !committed) {
            ...
            _delete_downloaded_files(download_success_files, "leader error cleanup", txn_id);
        }
    ...
    for (int64_t segment_index = 0; segment_index < num_segments; ++segment_index) {
        ...
        std::string* segment_file_md5_ptr = is_single_replica_download ? &segment_file_md5 : nullptr;
    ...
    if (commit_result == IngestCommitResult::kError) {
        status = Status::RuntimeError(
                "failed to commit ingested rowset, status={}", commit_result.status.to_string());
        status.to_thrift(&tstatus);
        return;
    }
    if (commit_result == IngestCommitResult::kAlreadyExist) {
...
void BackendService::ingest_binlog(TIngestBinlogResult& result,
    ...
    LOG(INFO) << "ingest binlog. txn_id=" << (request.__isset.txn_id ? request.txn_id : -1)
              ...
              << ", load_id="
              << (request.__isset.load_id ? print_id(request.load_id) : "not_set")
              << ", fetch_from_peer=" << (request.__isset.fetch_from_peer && request.fetch_from_peer)
